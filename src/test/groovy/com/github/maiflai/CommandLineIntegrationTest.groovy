...
class CommandLineIntegrationTest {
    ...
    void testSpecIsRun() throws Exception {
        setupBuild()
                .forTasks('clean', 'scalatest', '--tests', 'MySpec')
                .run()
    ...
    void testOnlyTestIsRun() throws Exception {
        setupBuild()
                .forTasks('clean', 'scalatest', '--tests', 'bob')
                .run()
    ...
    private static File testReport = new File(projectDir, 'build/test-results/scalatest/TEST-MySpec.xml')
